...
//Define basic linked list class using template
 template <class T>
 struct link_list
{
    T data;   //data field
...
//Define Stack class using templates
template <class T>
class Stack
{
 public :
    struct link_list<T> *top; //pointer to top of stack
 ...
 public :
  ...
  void push(int el)
  {
      if(this->top==NULL)
      {
          this->top=(struct link_list<T>*)malloc(sizeof(struct link_list<T>));
          this->top->data=el;
          this->top->next=NULL;
      }
      else
        {
           struct link_list<T>* node;
           node=(struct link_list<T>*)malloc(sizeof(struct link_list<T>)) ;
           node->data=el;
           node->next=this->top;
           this->top=node;
        }
 ...
 //pop from stack
   int pop()
   {
        if(!is_empty())
    {
        ...
        struct link_list<int>* to_free=this->top;
...
template <typename T>
void check_operator(string& post_str,char c,Stack<T>& s)
{
...
int main()
{
   ...
   cin>>infix_str;
   cout<<"entered infix expression=1++"<<endl;
   cout<<infix_str<<endl;
 ...
 for(int i=0;i<infix_str.size();i++)
  ...
  //add top of stack to postfix until stack is not empty
    ...
    cout<<eval_postfix<int>(postfix_str)<<endl; // evaluates value of postfix expression
    return 0;
}
